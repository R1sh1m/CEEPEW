...
static uint16_t s_frag_accum_len = 0U;
static bool s_frag_active = false;
static uint32_t s_frag_start_ms = 0U;
...
/* RX data pipeline handles and build-once flags */
static Pipeline_t s_rx_decrypt_pipeline;
static bool s_rx_decrypt_pipeline_built = false;
static Pipeline_t s_rx_decompress_pipeline;
static bool s_rx_decompress_pipeline_built = false;

static CeePewErr_t rxd_build_pipeline(void)
{
    CeePewErr_t err;

    err = pipeline_reset(&s_rx_decrypt_pipeline);
    if (err != CEEPEW_OK) { return err; }

    err = pipeline_add_stage(&s_rx_decrypt_pipeline, stage_rx_crc_fec, NULL);
    if (err != CEEPEW_OK) { return err; }

    err = pipeline_add_stage(&s_rx_decrypt_pipeline, stage_rx_decrypt, NULL);
    if (err != CEEPEW_OK) { return err; }

    s_rx_decrypt_pipeline_built = true;

    err = pipeline_reset(&s_rx_decompress_pipeline);
    if (err != CEEPEW_OK) { return err; }

    err = pipeline_add_stage(&s_rx_decompress_pipeline, stage_rx_decompress, NULL);
    if (err != CEEPEW_OK) { return err; }

    s_rx_decompress_pipeline_built = true;
    return CEEPEW_OK;
}
...
static CeePewErr_t process_rx_frame(const RadioFrame_t *frame) {
  ...
  /* ── RX decrypt pipeline: CRC → FEC decode → decrypt ────────
