 ...
 *
 * transport_ble_gatt_crypto.c: GATT commitment exchange crypto wrapper (Phase
 * 2). Provides gatt_crypto_encrypt_with_ids() / gatt_crypto_decrypt_with_ids()
...
static void transport_ble_log_peer_snapshot(const char *prefix,
  ...
  ESP_LOGI(TAG, "%s peer=%s rssi=%d seen_at=%lu name=%s hits=%u", prefix, mac,
           (int)peer->rssi, (unsigned long)peer->seen_at,
           (peer->name_len > 0U) ? name : "<none>",
...
static CeePewErr_t
transport_ble_set_commitment_beacon_unlocked(const uint8_t *commitment,
                                             uint8_t len);
...
static const uint8_t s_adv_raw_data[] = {
    2U,
    0x01U,
    (uint8_t)(ESP_BLE_ADV_FLAG_GEN_DISC | ESP_BLE_ADV_FLAG_BREDR_NOT_SPT),
    3U,
    0x03U,
    (uint8_t)(BLE_SERVICE_UUID & 0xFFU),
    (uint8_t)((BLE_SERVICE_UUID >> 8U) & 0xFFU),
    ...
    'E',
    'W'};
static const uint8_t s_scan_rsp_raw_data[] = {7U,  0x09U, 'C', 'E',
                                              'E', 'P',   'E', 'W'};
static esp_bt_uuid_t s_sign_pk_char_uuid = {
    .len = ESP_UUID_LEN_16, .uuid = {.uuid16 = BLE_SIGN_PK_CHAR_UUID}};

/* Attribute value buffer for the sign_pk characteristic (matches
 * GATT_CRYPTO_TOTAL_BYTES). Must accommodate 70B ciphertext + 16B tag = 86B
 * total. Explicit max_length ensures GATT write succeeds for full payload. */
...
static esp_bt_uuid_t s_service_uuid_filter = {
    .len = ESP_UUID_LEN_16, .uuid = {.uuid16 = BLE_SERVICE_UUID}};
...
static uint32_t s_sign_pk_write_delay_ms =
    50U; /* delay carried to timer callback */
...
static bool s_local_mac_overridden =
    false; /* set if caller provided local_mac explicitly */
 ...
 * stale/unowned give is dropped instead of crashing the system. */
  ...
  if (xSemaphoreGetMutexHolder(s_ble_init_mutex) !=
      xTaskGetCurrentTaskHandle()) {
    ESP_LOGW(
        TAG,
        "ble_init_unlock: dropping give — mutex not owned by current task");
    return;
  }
#endif
...
bool transport_ble_ctx_lock(void) { return ble_ctx_lock(); }

void transport_ble_ctx_unlock(void) { ble_ctx_unlock(); }
...
static bool s_stack_needs_full_init =
    true; /* false after first full init; set true by deinit */

static void transport_ble_log_state_snapshot(const char *prefix) {
  char mac[18];
  transport_ble_format_mac(g_ble_ctx.peer_mac, mac);
  ESP_LOGI(TAG,
           "%s: state=%s discovered=%u gattc=%u gatts=%u conn_id=%u "
           "local_len=%u peer_pending=%u beacon=%u sign_pk=%u reconnects=%u",
           prefix, transport_ble_state_name(g_ble_ctx.state),
           g_ble_ctx.discovered ? 1U : 0U, g_ble_ctx.gattc_connected ? 1U : 0U,
           g_ble_ctx.gatts_connected ? 1U : 0U, (unsigned)g_ble_ctx.conn_id,
...
CeePewErr_t transport_ble_restart_discovery_session(void) {
  ...
  for (int attempt = 0; attempt < 3; ++attempt) {
    if (s_ble_initialised) {
      ESP_LOGI(TAG,
               "restart_discovery: tearing down BLE for fresh GATTS creation "
               "(attempt %d/3)",
               attempt + 1);
    ...
    if (init_err == CEEPEW_ERR_BUSY) {
      ESP_LOGW(TAG,
               "restart_discovery: init lock busy, retrying in 200 ms (attempt "
               "%d/3)",
               attempt + 1);
  ...
  if (init_err != CEEPEW_OK) {
    ESP_LOGE(TAG, "Failed to re-initialize BLE after retries: %d",
             (int)init_err);
    return init_err;
  }
  ...
  if (err != CEEPEW_OK) {
    ESP_LOGW(TAG, "session_restart_discovery failed during pairing reset: %d",
             (int)err);
    return err;
  }
...
#define CEEPEW_SCAN_DEDUPE_TABLE_SIZE 16U
...
static ScanDedupeEntry_t s_scan_dedupe_table[CEEPEW_SCAN_DEDUPE_TABLE_SIZE] = {0};

static void transport_ble_scan_dedupe_clear(void) {
  memset(s_scan_dedupe_table, 0U, sizeof(s_scan_dedupe_table));
}
...
static void transport_ble_clear_discovery_peer_state_unlocked(void) {
  ...
  ceepew_secure_zero(g_ble_ctx.adv_commitment,
                     sizeof(g_ble_ctx.adv_commitment));
  ...
  transport_ble_scan_dedupe_clear();
...
static void peer_cache_upsert(const uint8_t mac[6], int8_t rssi,
  ...
  if (idx >= 0) {
    ...
    g_ble_ctx.peer_cache[idx].record.seen_at =
    ...
    if (has_beacon && commitment) {
      memcpy(g_ble_ctx.peer_cache[idx].peer_commitment, commitment,
  ...
  g_ble_ctx.peer_cache[new_idx].record.seen_at =
      (uint32_t)(esp_timer_get_time() / 1000000LL);
  if (name_len > 0U) {
  ...
  if (has_beacon && commitment) {
    memcpy(g_ble_ctx.peer_cache[new_idx].peer_commitment, commitment,
           CEEPEW_COMMITMENT_ADV_BYTES);
  }
...
void transport_ble_peer_cache_expire_ttl(uint32_t now_ms) {
  ...
  for (uint8_t i = 0U; i < g_ble_ctx.peer_cache_count; i++) {
    if (now_ms - g_ble_ctx.peer_cache[i].cached_at_ms <
        (CEEPEW_PEER_CACHE_TTL_S * 1000U)) {
      if (write_idx != i) {
...
CeePewErr_t transport_ble_set_selected_peer(uint8_t cache_index) {
  ...
  memcpy(g_ble_ctx.peer_mac, g_ble_ctx.peer_cache[cache_index].record.peer_mac,
         6U);
  g_ble_ctx.peer_addr_type = BLE_ADDR_TYPE_PUBLIC;
...
static bool transport_ble_peer_is_recent_at(uint32_t now_ms,
                                            uint32_t max_age_ms) {
...
static const char *transport_ble_hci_reason_name(uint8_t reason) {
  ...
  case 0x3B:
    return "LOCAL_HOST_TERMINATED_CONN";
  case 0x3E:
...
static bool transport_ble_scan_peer_is_duplicate(const uint8_t mac[6U],
  ...
  uint8_t free_idx = CEEPEW_SCAN_DEDUPE_TABLE_SIZE;

  for (uint8_t i = 0U; i < CEEPEW_SCAN_DEDUPE_TABLE_SIZE; i++) {
    if (s_scan_dedupe_table[i].valid) {
      if (ceepew_ct_equal(s_scan_dedupe_table[i].mac, mac, 6U)) {
        if ((uint32_t)(now_ms - s_scan_dedupe_table[i].seen_ms) <=
            CEEPEW_SCAN_PEER_DEDUPE_WINDOW_MS) {
          return true;
        }
        s_scan_dedupe_table[i].seen_ms = now_ms;
        return false;
      }
      if (s_scan_dedupe_table[i].seen_ms < oldest_time) {
        oldest_time = s_scan_dedupe_table[i].seen_ms;
        oldest_idx = i;
      }
    } else if (free_idx == CEEPEW_SCAN_DEDUPE_TABLE_SIZE) {
      free_idx = i;
    }
  }

  uint8_t target_idx =
      (free_idx < CEEPEW_SCAN_DEDUPE_TABLE_SIZE) ? free_idx : oldest_idx;
  memcpy(s_scan_dedupe_table[target_idx].mac, mac, 6U);
  s_scan_dedupe_table[target_idx].seen_ms = now_ms;
  s_scan_dedupe_table[target_idx].valid = true;
  return false;
}
...
static bool transport_ble_payload_contains(const uint8_t *buf, uint16_t buf_len,
 ...
 */
static CeePewErr_t transport_ble_configure_data_raw_unlocked(
    const uint8_t *adv_data, uint8_t adv_len, const uint8_t *scan_rsp_data,
  ...
  if (g_ble_ctx.is_advertising) {
    ...
    ESP_LOGI(TAG, "configure_data_raw: advertising active — stopping first to "
                  "apply new payloads");
    ...
    if (err != ESP_OK) {
      ...
      if (scan_rsp_data && scan_rsp_len > 0) {
        esp_ble_gap_config_scan_rsp_data_raw((uint8_t *)scan_rsp_data,
                                             scan_rsp_len);
      }
    }
  } else {
    ESP_LOGI(TAG, "configure_data_raw: advertising inactive — applying "
                  "payloads directly");
    if (adv_data && adv_len > 0) {
      s_adv_data_set = false;
      esp_ble_gap_config_adv_data_raw((uint8_t *)adv_data, adv_len);
...
transport_ble_set_commitment_beacon_unlocked(const uint8_t *commitment,
  ...
  CEEPEW_ASSERT(len > 0U && len <= CEEPEW_COMMITMENT_ADV_BYTES,
                CEEPEW_ERR_PARAM);
  ...
  s_commitment_buf[pos++] = 'C';
  s_commitment_buf[pos++] = 'E';
  s_commitment_buf[pos++] = 'E';
  s_commitment_buf[pos++] = 'P';
  s_commitment_buf[pos++] = 'E';
  s_commitment_buf[pos++] = 'W';
   ...
   * counter. */
  if (g_ble_ctx.beacon_nonce_local == 0U) {
    g_ble_ctx.beacon_nonce_local = 1U;
  }
  ...
  /* Ensure we do not exceed the standard BLE advertising payload limit of 31
   * bytes */
  ...
  ESP_LOGI(
      TAG,
      "Commitment beacon queued (%u bytes, wire_nonce=0x%04x, gatt_ready=%u)",
...
CeePewErr_t transport_ble_set_commitment_beacon(const uint8_t *commitment,
                                                uint8_t len) {
  CEEPEW_ASSERT(commitment != NULL, CEEPEW_ERR_NULL_PTR);
  CEEPEW_ASSERT(len > 0U && len <= CEEPEW_COMMITMENT_ADV_BYTES,
                CEEPEW_ERR_PARAM);
  ...
  ble_ctx_lock();
  CeePewErr_t err =
      transport_ble_set_commitment_beacon_unlocked(commitment, len);
  ble_ctx_unlock();
  return err;
}

/* Send HANDOFF_READY beacon (subtype 0x03) to signal PIN confirmed and ready
 * for ESP-NOW handoff. This beacon allows both peers to synchronize before
 * tearing down BLE and initializing ESP-NOW. Beacon format: AD name "CEEPEW" +
 * manufacturer AD 0xCEEE subtype 0x03 + magic bytes 0xCE 0xEF 0x03 Total: 8
 * (name) + 7 (mfr: len=5 + company 2 + subtype 1 + magic 3) = 15 bytes */
...
CeePewErr_t transport_ble_init(void) {
  ...
  if (!ble_init_lock()) {
    ESP_LOGE(TAG, "transport_ble_init: init lock unavailable (concurrent "
                  "init/deinit) — aborting");
    return CEEPEW_ERR_BUSY;
  }
  ...
  memset(g_ble_ctx.pending_sign_pk_encrypted, 0U,
         sizeof(g_ble_ctx.pending_sign_pk_encrypted));
  memset(g_ble_ctx.pending_peer_commitment, 0U,
         sizeof(g_ble_ctx.pending_peer_commitment));
  /* Diagnostics (Phase 8) */
  ...
  if (s_stack_needs_full_init ||
    ...
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
      ESP_LOGE(TAG, "esp_bt_controller_init FAILED: %d (%s)", err,
               esp_err_to_name(err));
      ble_init_unlock();
      return CEEPEW_ERR_HW;
    }
    ...
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
      ESP_LOGE(TAG, "esp_bt_controller_enable FAILED: %d (%s)", err,
               esp_err_to_name(err));
      ble_init_unlock();
      return CEEPEW_ERR_HW;
    }
    ...
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
      ESP_LOGE(TAG, "esp_bluedroid_init_with_cfg FAILED: %d (%s)", err,
               esp_err_to_name(err));
      ble_init_unlock();
      return CEEPEW_ERR_HW;
    }
    ...
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
      ESP_LOGE(TAG, "esp_bluedroid_enable FAILED: %d (%s)", err,
               esp_err_to_name(err));
      ble_init_unlock();
      return CEEPEW_ERR_HW;
    }
    ...
    ESP_LOGI(
        TAG,
        "BLE stack already initialised — skipping controller+bluedroid init");
  }
  ...
  if (s_sign_pk_delay_timer == NULL) {
    const esp_timer_create_args_t timer_args = {
        .callback = sign_pk_delayed_dispatch, .name = "sign_pk_delay"};
    esp_err_t t_err = esp_timer_create(&timer_args, &s_sign_pk_delay_timer);
    if (t_err != ESP_OK) {
      ESP_LOGE(TAG, "sign_pk delay timer create failed: %d (%s)", (int)t_err,
  ...
  /* Set initialization flag BEFORE registering callbacks to avoid race
   * condition. */
  ...
  if (err == ESP_ERR_INVALID_STATE) {
    ...
    ESP_LOGE(TAG, "esp_ble_gattc_app_register failed: %d (%s)", err,
             esp_err_to_name(err));
    ble_init_unlock();
    return CEEPEW_ERR_HW;
  }
  ...
  if (err == ESP_ERR_INVALID_STATE) {
    ...
    ESP_LOGE(TAG, "esp_ble_gatts_app_register failed: %d (%s)", err,
             esp_err_to_name(err));
  ...
  if (sup_err != CEEPEW_OK) {
    ESP_LOGW(TAG, "transport_ble_init: supervisor start failed: %d",
             (int)sup_err);
  }
...
CeePewErr_t transport_ble_start_advertising(void) {
  ...
  if (g_ble_ctx.state != BLE_IDLE && g_ble_ctx.state != BLE_ADVERTISING &&
      g_ble_ctx.state != BLE_SCANNING &&
      g_ble_ctx.state != BLE_ADVERTISING_AND_SCANNING) {
    return CEEPEW_ERR_BUSY;
  }
...
#ifdef CONFIG_BT_ENABLED
  CeePewErr_t cfg_err = transport_ble_configure_data_raw_unlocked(
      s_adv_raw_data, sizeof(s_adv_raw_data), s_scan_rsp_raw_data,
  ...
  ESP_LOGI(TAG,
           "Advertisement and scan response configured; awaiting GAP callbacks "
           "(svc=0x%04X)",
...
CeePewErr_t transport_ble_start_scan(void) {
  ...
  if (g_ble_ctx.is_scanning || g_ble_ctx.state == BLE_SCANNING ||
      g_ble_ctx.state == BLE_ADVERTISING_AND_SCANNING) {
  ...
  ESP_LOGI(
      TAG,
      "Scan requested: active=%u interval=%u window=%u duplicate=%u bt_stat=%d",
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "esp_ble_gap_set_scan_params failed: %d (%s) bt_stat=%d", err,
             esp_err_to_name(err), (int)bt_stat);
    s_scan_requested = false;
    s_scan_start_failed = true;
    s_scan_retry_after_ms =
        (uint32_t)(esp_timer_get_time() / 1000LL) + CEEPEW_RESTART_DEBOUNCE_MS;
    return CEEPEW_ERR_HW;
  }
  s_scan_requested = true;
  ESP_LOGI(TAG,
           "Scan parameters set — waiting for SCAN_PARAM_SET_COMPLETE_EVT");
#endif

  ESP_LOGI(
      TAG,
      "Scan params sent — waiting for SCAN_PARAM_SET_COMPLETE_EVT (state=%s)",
      transport_ble_state_name(g_ble_ctx.state));
  return CEEPEW_OK;
}

BleState_t transport_ble_get_state(void) { return g_ble_ctx.state; }

const BlePeerRecord_t *transport_ble_get_peer(void) {
  uint32_t now_ms = (uint32_t)(esp_timer_get_time() / 1000ULL);
  if (!transport_ble_peer_is_recent_at(now_ms,
                                       CEEPEW_DISCOVERY_PEER_VISIBLE_MS)) {
    return NULL;
  }
...
static CeePewErr_t
transport_ble_connect_to_peer_internal(const uint8_t peer_mac[6],
                                       bool is_retry) {
  if (peer_mac == NULL) {
...
#ifdef CONFIG_BT_ENABLED
  if (g_ble_ctx.gattc_if == CEEPEW_GATT_IF_NONE ||
      !g_ble_ctx.gattc_registered) {
  ...
  /* ESP-IDF v6: Connection parameters are now in a separate
   * esp_ble_conn_params_t struct pointed to by phy_1m_conn_params in
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "esp_ble_gattc_enh_open failed: %d (%s)", err,
             esp_err_to_name(err));
    g_ble_ctx.connecting = false;
    return CEEPEW_ERR_HW;
  }
...
static CeePewErr_t
transport_ble_connect_to_peer_unlocked(const uint8_t peer_mac[6]) {
...
static CeePewErr_t
transport_ble_verify_commitment_unlocked(const uint8_t *peer_digest,
                                         uint8_t len) {
  if (peer_digest == NULL) {
    return CEEPEW_ERR_NULL_PTR;
  }
  if (session_get_phase() != 2U) {
    if (g_ble_ctx.state != BLE_PAIRING && g_ble_ctx.state != BLE_CONNECTED &&
        g_ble_ctx.state != BLE_ADVERTISING_AND_SCANNING &&
        g_ble_ctx.state != BLE_SCANNING && g_ble_ctx.state != BLE_ADVERTISING) {
      return CEEPEW_ERR_PARAM;
    }
  }
  ...
  if (g_ble_ctx.commitment_beacon_active) {
    CeePewErr_t reb_err = transport_ble_set_commitment_beacon_unlocked(
        g_ble_ctx.adv_commitment, CEEPEW_COMMITMENT_ADV_BYTES);
  ...
  (void)rgb_set_pattern(
      transport_ble_phase_to_rgb(PAIRING_PHASE_GATT_IDENTITY));
  ESP_LOGI(TAG, "GATT-ready rebroadcast dispatched, entering GATT_IDENTITY");

  return CEEPEW_OK;
}

CeePewErr_t transport_ble_verify_commitment(const uint8_t *peer_digest,
                                            uint8_t len) {
  ble_ctx_lock();
...
static CeePewErr_t transport_ble_verify_pending_commitment_unlocked(void) {
  ...
  ESP_LOGI(
      TAG, "verify_pending: phase=%d pending_len=%u commit=%02X%02X%02X%02X...",
      session_get_phase(), pending_len, g_ble_ctx.pending_peer_commitment[0],
      g_ble_ctx.pending_peer_commitment[1],
      g_ble_ctx.pending_peer_commitment[2],
      g_ble_ctx.pending_peer_commitment[3]);
   ...
   * alongside the "CEEPEW" name (13 bytes total, well under 31). */
  {
    ...
    reject_buf[rpos++] =
        4U; /* length: 2 (company) + 1 (subtype) + 1 (reason) */
    ...
    (void)transport_ble_configure_data_raw_unlocked(NULL, 0U, reject_buf, rpos);
  }
...
static CeePewErr_t transport_ble_disconnect_unlocked(void) {
  ...
  if (!s_discovery_restart_pending && !g_ble_ctx.commitment_verified &&
      !session_is_active() && pre_disconnect_state != BLE_PAIRING &&
      pre_disconnect_state != BLE_CONNECTED) {
    s_adv_data_set = false;
    s_scan_rsp_set = false;
    s_adv_starting = false;
    (void)transport_ble_start_advertising();
...
CeePewErr_t transport_ble_deinit(void) {
  ...
  if (!ble_init_lock()) {
    ESP_LOGE(TAG, "transport_ble_deinit: init lock unavailable (concurrent "
                  "init/deinit) — aborting");
    return CEEPEW_ERR_BUSY;
  }
  ...
  ESP_LOGI(
      TAG,
      "BLE deinit: stack torn down (memory retained for potential reinit)");
#else
  ...
  s_last_restart_ms =
      0U; /* Clear debounce timer so restart_discovery_session() can re-init */
  ble_init_unlock();
  return CEEPEW_OK;
}
...
CeePewErr_t transport_ble_teardown(void) {
  ...
  if (!ble_init_lock()) {
    ESP_LOGE(TAG, "transport_ble_teardown: init lock unavailable (concurrent "
                  "init/deinit) — aborting");
    return CEEPEW_ERR_BUSY;
  }
  ...
  /* 2. Disconnect any active GATT links cleanly via esp_ble_gatts_close() and
   * esp_ble_gattc_close() */
  if (g_ble_ctx.gattc_connected) {
    esp_ble_gattc_close(g_ble_ctx.gattc_if, g_ble_ctx.conn_id);
    vTaskDelay(pdMS_TO_TICKS(100));
  ...
  /* 7. Mandatory blocking delay to let the shared 2.4GHz PHY radio fully settle
   * into an idle state */
...
bool transport_ble_is_initialised(void) { return s_ble_initialised; }

CeePewErr_t transport_ble_set_scan_duty_cycle(uint16_t interval_ms,
                                              uint16_t window_ms) {
#ifdef CONFIG_BT_ENABLED
  if (!s_ble_initialised) {
    return CEEPEW_OK;
  ...
  if (g_ble_ctx.is_scanning) {
    ESP_LOGI(TAG,
             "BLE scan duty (deferred): interval=%u ms, window=%u ms (%u%%)",
  ...
  esp_err_t err = esp_ble_gap_set_scan_params(&scan_params);
  if (err != ESP_OK) {
    ESP_LOGW(TAG, "set_scan_duty_cycle failed: %d (%s)", err,
             esp_err_to_name(err));
    return CEEPEW_ERR_HW;
  }
...
CeePewErr_t transport_ble_retry_scan_if_needed(void) {
  ...
  if (esp_bluedroid_get_status() == ESP_BLUEDROID_STATUS_UNINITIALIZED) {
    if (g_ble_ctx.state == BLE_DONE || session_get_phase() >= 2U ||
        session_is_active()) {
      return CEEPEW_OK;
    }
  ...
  {
    bool skip_clear = g_ble_ctx.gattc_connected || g_ble_ctx.gatts_connected ||
                      g_ble_ctx.commitment_verified ||
                      g_ble_ctx.handoff_ready || session_is_active();
    ...
    if (!skip_clear && g_ble_ctx.discovered && g_ble_ctx.last_seen_ms != 0U &&
        !transport_ble_peer_is_recent_at(now_ms, PEER_LOST_TIMEOUT_MS)) {

      ESP_LOGW(TAG,
               "Peer truly lost: last seen %lu ms ago — clearing discovered",
  ...
  if (g_ble_ctx.is_scanning || g_ble_ctx.state == BLE_SCANNING ||
    ...
    if (seen != s_last_scan_seen_value) {
      ...
      uint32_t idle = (now_ms > s_last_scan_seen_change_ms)
                          ? (now_ms - s_last_scan_seen_change_ms)
                          : 0U;
      if (idle > CEEPEW_SCAN_STUCK_TIMEOUT_MS && !discovered) {
        ESP_LOGW(TAG,
                 "BLE scan appears stalled (%lu ms no new peers) — restarting "
                 "discovery",
                 (unsigned long)idle);
        CeePewErr_t restart_err = transport_ble_restart_discovery_session();
        if (restart_err != CEEPEW_OK) {
          ESP_LOGE(TAG, "Discovery restart failed during scan-watchdog: %d",
                   (int)restart_err);
          return restart_err;
        }
        s_last_scan_seen_change_ms = now_ms;
...
static void gap_event_handler(esp_gap_ble_cb_event_t event,
  ...
  case ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT: {
    ...
    if (s_adv_data_set && s_scan_rsp_set && !s_adv_starting) {
      ...
      if (e != ESP_OK) {
        ESP_LOGE(
            TAG,
            "esp_ble_gap_start_advertising failed (from ADV_DATA_SET): %d (%s)",
            e, esp_err_to_name(e));
        s_adv_starting = false;
      } else {
  ...
  case ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT:
    s_scan_rsp_set = true;
    ESP_LOGI(TAG, "SCAN_RSP_DATA_SET_COMPLETE (adv_data_ready=%u)",
             s_adv_data_set);
    ...
    if (s_adv_data_set && s_scan_rsp_set && !s_adv_starting) {
      ...
      if (e != ESP_OK) {
        ESP_LOGE(
            TAG,
            "esp_ble_gap_start_advertising failed (from SCAN_RSP_SET): %d (%s)",
            e, esp_err_to_name(e));
        s_adv_starting = false;
      } else {
  ...
  case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
    ...
    if (param->scan_param_cmpl.status != ESP_BT_STATUS_SUCCESS) {
      if ((param->scan_param_cmpl.status == 0x0CU) &&
          (g_ble_ctx.is_scanning || g_ble_ctx.state == BLE_SCANNING ||
           g_ble_ctx.state == BLE_ADVERTISING_AND_SCANNING)) {
        ESP_LOGW(TAG, "Scan param set returned Cmd Disallowed while scan "
    ...
    }
    {
      ...
      if (e != ESP_OK) {
        ESP_LOGE(TAG, "esp_ble_gap_start_scanning FAILED: %d (%s)", e,
                 esp_err_to_name(e));
        s_scan_requested = false;
        s_scan_start_failed = true;
        s_scan_retry_after_ms = (uint32_t)(esp_timer_get_time() / 1000LL) +
  ...
  case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
    ...
    if (param->scan_start_cmpl.status == ESP_BT_STATUS_SUCCESS) {
      ...
      if ((param->scan_start_cmpl.status == 0x0CU) &&
        ...
        ESP_LOGW(
            TAG,
            "Scan start Cmd Disallowed while scan already active; ignoring");
        s_scan_requested = false;
        s_scan_start_failed = false;
        s_scan_retry_after_ms = 0U;
                 ...
                 g_ble_ctx.state == BLE_ADVERTISING_AND_SCANNING) {
        ESP_LOGW(TAG,
                 "Scan start failed (status=%d) but scan already active — "
  ...
  case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT:
    ...
    if (s_pending_adv_update) {
      ...
      s_adv_data_set = false;
      ESP_LOGI(TAG,
               "ADV_STOP_COMPLETE: configuring pending raw adv data (%u bytes)",
               s_pending_adv_len);
    ...
    if (s_pending_scan_rsp_update) {
      ...
      s_scan_rsp_set = false;
      ESP_LOGI(TAG,
               "ADV_STOP_COMPLETE: configuring pending raw scan response data "
               "(%u bytes)",
               s_pending_scan_rsp_len);
      esp_ble_gap_config_scan_rsp_data_raw(s_pending_scan_rsp_buf,
                                           s_pending_scan_rsp_len);
    }
    break;
  ...
  case ESP_GAP_BLE_SCAN_RESULT_EVT: {
    ...
    if (g_ble_ctx.scan_seen_count < UINT32_MAX) {
      g_ble_ctx.scan_seen_count++;
    }

    if (g_ble_ctx.scan_seen_count % 100U == 0U) {
      ESP_LOGI(TAG,
               "Scan activity: seen_count=%lu discovered=%u hits=%u state=%s",
               (unsigned long)g_ble_ctx.scan_seen_count,
               g_ble_ctx.discovered ? 1U : 0U, g_ble_ctx.scan_hit_count,
               transport_ble_state_name(g_ble_ctx.state));
    }
    ...
    if (raw_adv != NULL && total_adv_len >= 6U) {
      payload_has_ceepew_name = transport_ble_payload_contains(
          raw_adv, total_adv_len, (const uint8_t *)"CEEPEW", 6U);
    }
    uint8_t *name_ptr = esp_ble_resolve_adv_data_by_type(
        raw_adv, total_adv_len, ESP_BLE_AD_TYPE_NAME_CMPL, &found_name_len);
    if (name_ptr == NULL) {
      name_ptr = esp_ble_resolve_adv_data_by_type(
          raw_adv, total_adv_len, ESP_BLE_AD_TYPE_NAME_SHORT, &found_name_len);
    }
    ...
    uint8_t *uuid_ptr = esp_ble_resolve_adv_data_by_type(
        raw_adv, total_adv_len, ESP_BLE_AD_TYPE_16SRV_CMPL, &service_uuid_len);
    if (uuid_ptr == NULL) {
      uuid_ptr = esp_ble_resolve_adv_data_by_type(raw_adv, total_adv_len,
                                                  ESP_BLE_AD_TYPE_16SRV_PART,
                                                  &service_uuid_len);
    }
    if (uuid_ptr != NULL && service_uuid_len >= sizeof(s_adv_service_uuid16) &&
        memcmp(uuid_ptr, s_adv_service_uuid16, sizeof(s_adv_service_uuid16)) ==
            0) {
      service_uuid_match = true;
    }

    /* Also check for manufacturer-specific company ID 0xCEEE in scan response
     * data */
    bool mfr_ceepew = false;
    if (raw_adv != NULL && total_adv_len >= 6U) {
      uint8_t mfr_len = 0U;
      uint8_t *mfr_ptr = esp_ble_resolve_adv_data_by_type(
          raw_adv, total_adv_len, ESP_BLE_AD_MANUFACTURER_SPECIFIC_TYPE,
          &mfr_len);
      if (mfr_ptr != NULL && mfr_len >= 2U && mfr_ptr[0] == 0xEEU &&
          mfr_ptr[1] == 0xCEU) {
        mfr_ceepew = true;
      }
    }
    ...
    bool is_ceepew_peer =
        ((found_name_len >= 6U && memcmp(found_name, "CEEPEW", 6U) == 0) ||
         service_uuid_match || payload_has_ceepew_name || mfr_ceepew);

    /* Filter out other signals early so non-CEEPEW devices do not overwhelm
     * CPU cycles, serial logs, or the peer discovery table. */
    if (!is_ceepew_peer) {
      break;
    }
    ...
    uint32_t keep_ms = in_pairing_flow ? CEEPEW_PAIRING_PEER_KEEP_MS
                                       : CEEPEW_DISCOVERY_PEER_CLEAR_MS;
    if (g_ble_ctx.discovered && !g_ble_ctx.gattc_connected &&
        !g_ble_ctx.gatts_connected && !g_ble_ctx.commitment_verified &&
        !g_ble_ctx.handoff_ready && !session_is_active() &&
        !transport_ble_peer_is_recent_at(now_ms, keep_ms)) {
      ESP_LOGI(TAG,
               "Cached peer expired after %lu ms — clearing discovery cache",
               (unsigned long)(now_ms - g_ble_ctx.last_seen_ms));
      transport_ble_clear_discovery_peer_state();
    }

    bool duplicate_same_peer = false;
    if (transport_ble_scan_peer_is_duplicate(param->scan_rst.bda, now_ms)) {
      duplicate_same_peer =
    ...
    /* Diagnostic: dump raw advertising bytes for every 200th CEEPEW scan result
     * to verify payload integrity during discovery. */
    if (g_ble_ctx.scan_seen_count % 200U == 1U) {
      if (raw_adv != NULL && total_adv_len > 0U) {
        ...
        ESP_LOGI(TAG,
                 "SCAN_DUMP[%lu]: peer=%s rssi=%d adv_len=%u rsp_len=%u "
                 "raw[0..%u]=%s",
    ...
    ESP_LOGD(TAG,
             "Scan signal: evt=%u peer=%s rssi=%d adv_len=%u name=%s svc=%s "
             "mfr=%s match=ceepew",
             (unsigned)param->scan_rst.search_evt, found_mac,
             (int)param->scan_rst.rssi, (unsigned)param->scan_rst.adv_data_len,
    ...
    if (is_ceepew_peer) {
      mfr_ptr = esp_ble_resolve_adv_data_by_type(raw_adv, total_adv_len, 0xFFU,
                                                 &mfr_len);
    }
    ...
    if (mfr_ptr != NULL &&
        mfr_len == (uint8_t)(3U + CEEPEW_COMMITMENT_ADV_BYTES) &&
    ...
    peer_cache_upsert(param->scan_rst.bda, param->scan_rst.rssi,
                      (uint8_t *)found_name, found_name_len, has_beacon,
                      has_beacon ? &mfr_ptr[5] : NULL);

    /* Update primary peer for backward compatibility (first discovered or
     * selected) */
    if (!g_ble_ctx.discovered) {
      ESP_LOGI(TAG,
               "CEEPEW peer first seen: %02X:%02X:%02X:%02X:%02X:%02X "
               "RSSI=%d name=%s",
      ...
      g_ble_ctx.peer_addr_type =
          (esp_ble_addr_type_t)param->scan_rst.ble_addr_type;
    ...
    if (is_ceepew_peer && cur_phase == 2U && !g_ble_ctx.commitment_verified &&
        mfr_ptr != NULL && mfr_len >= 3U && mfr_ptr[0] == 0xEEU &&
        mfr_ptr[1] == 0xCEU && mfr_ptr[2] == 0x51U) {
      ESP_LOGW(TAG, "Rejection beacon received from %s (reason=%d)", found_mac,
               (int)mfr_ptr[3]);
      session_ui_ctx_lock();
    ...
    if (is_ceepew_peer && (cur_phase == 1U || cur_phase == 2U) &&
      ...
      if (mfr_ptr != NULL &&
         ...
         * false rejection. */
        uint16_t peer_wire = ((uint16_t)mfr_ptr[3] << 8) | (uint16_t)mfr_ptr[4];
        ...
        if (nonce_ok && !g_ble_ctx.commitment_verified &&
          ...
          memcpy(g_ble_ctx.pending_peer_commitment, &mfr_ptr[5],
                 CEEPEW_COMMITMENT_ADV_BYTES);
          g_ble_ctx.pending_peer_commitment_len = CEEPEW_COMMITMENT_ADV_BYTES;
          g_ble_ctx.peer_commitment_pending = true;
          g_ble_ctx.peer_commitment_via_adv = true;
    ...
    /* HANDOFF_READY beacon (subtype 0x03): peer has confirmed PIN and is ready
     * for ESP-NOW handoff. We push an event so the pairing supervisor can
     * synchronize both sides before BLE teardown. */
    if (is_ceepew_peer && (cur_phase == 2U || cur_phase == 3U) &&
        mfr_ptr != NULL && mfr_len >= 4U && mfr_ptr[0] == 0xEEU &&
        mfr_ptr[1] == 0xCEU && mfr_ptr[2] == 0x03U) {
      /* Verify magic bytes: 0xCE 0xEF 0x03 */
      if (mfr_len >= 7U && mfr_ptr[3] == 0xCEU && mfr_ptr[4] == 0xEFU &&
          mfr_ptr[5] == 0x03U) {
        ESP_LOGI(TAG, "HANDOFF_READY beacon received from %s", found_mac);
        transport_ble_post_event(PAIRING_EVENT_HANDOFF_READY,
                                 param->scan_rst.bda, 6U);
      }
    }
  } break;
  ...
  case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT: {
    ...
    if (param->update_conn_params.status == ESP_BT_STATUS_SUCCESS) {
      ESP_LOGI(
          TAG,
          "Conn params updated: int_min=%d int_max=%d latency=%d timeout=%d",
 ...
 * settle before the sign_pk write. The callback posts a
 * PAIRING_EVENT_SIGN_PK_WRITE event to the supervisor queue with the settle
 * delay as payload. The actual write is performed on the supervisor task (Core
 * 1) under ctx lock, avoiding BLE API calls from the esp_timer task context. */
static void sign_pk_delayed_dispatch(void *arg) {
  (void)arg;
  ...
  transport_ble_post_event(PAIRING_EVENT_SIGN_PK_WRITE, payload,
                           sizeof(payload));
}
...
static void gattc_event_handler(esp_gattc_cb_event_t event,
  ...
  case ESP_GATTC_REG_EVT:
    ...
    ESP_LOGI(
        TAG,
        "GATTC registered, if=%d — deferring scan until adv start complete",
        gattc_if);
    s_scan_start_pending = true;
    break;
  ...
  case ESP_GATTC_OPEN_EVT:
    ESP_LOGI(TAG,
             "GATTC_OPEN_EVT: status=%d conn_id=%u "
             "is_init=%d rev_pend=%d init_sent=%d",
             param->open.status, param->open.conn_id,
             g_ble_ctx.is_initiator_role, g_ble_ctx.reverse_gattc_pending,
    ...
    if (param->open.status != ESP_GATT_OK) {
      ...
      if (mtu_err != ESP_OK) {
        ESP_LOGW(TAG, "GATTC MTU request failed: %d (%s)", (int)mtu_err,
  ...
  case ESP_GATTC_CFG_MTU_EVT:
    if (param->cfg_mtu.status == ESP_GATT_OK) {
      ...
      ESP_LOGI(
          TAG,
          "GATTC MTU negotiated: %u (need >= 90 for sign_pk+box_pk+wifi_mac)",
          (unsigned)g_ble_ctx.gattc_mtu);
      if (g_ble_ctx.gattc_mtu < GATT_MIN_MTU) {
        ESP_LOGW(
            TAG,
            "MTU %u below sign_pk+box_pk threshold — write will be rejected",
            (unsigned)g_ble_ctx.gattc_mtu);
        ...
        if (!g_ble_ctx.gattc_connected) {
          ESP_LOGW(
              TAG,
              "Deferred sign_pk write: connection already gone, discarding");
          ceepew_secure_zero(g_ble_ctx.pending_sign_pk_encrypted,
                             sizeof(g_ble_ctx.pending_sign_pk_encrypted));
          g_ble_ctx.pending_sign_pk_write = false;
        } else {
          ...
          (void)transport_ble_post_event(PAIRING_EVENT_SIGN_PK_WRITE, payload,
                                         sizeof(payload));
        }
      }
    } else {
      ...
      if (g_ble_ctx.mtu_negotiation_attempts < 2U &&
        ...
        esp_err_t mtu_err =
            esp_ble_gattc_send_mtu_req(gattc_if, param->cfg_mtu.mtu);
        if (mtu_err != ESP_OK) {
        ...
        ESP_LOGW(TAG,
                 "MTU negotiation failed after %u attempts — discarding "
                 "buffered sign_pk write",
                 (unsigned)g_ble_ctx.mtu_negotiation_attempts);
        ceepew_secure_zero(g_ble_ctx.pending_sign_pk_encrypted,
                           sizeof(g_ble_ctx.pending_sign_pk_encrypted));
        g_ble_ctx.pending_sign_pk_write = false;
  ...
  case ESP_GATTC_CONNECT_EVT:
    ...
    g_ble_ctx.gattc_connected = true;
    g_ble_ctx.gatt_connected_since_ms =
  ...
  case ESP_GATTC_SEARCH_RES_EVT:
    if (param->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 &&
      ...
      ESP_LOGI(TAG, "GATTC service found: %u-%u",
               g_ble_ctx.service_start_handle, g_ble_ctx.service_end_handle);
    }
    break;

  case ESP_GATTC_SEARCH_CMPL_EVT: {
    if (param->search_cmpl.status != ESP_GATT_OK) {
      ESP_LOGE(TAG, "GATTC service search failed: %d",
    ...
    esp_err_t err = esp_ble_gattc_get_char_by_uuid(
        gattc_if, g_ble_ctx.conn_id, g_ble_ctx.service_start_handle,
        g_ble_ctx.service_end_handle, s_sign_pk_char_uuid, &char_elem, &count);
    if (err != ESP_OK || count == 0U) {
      ESP_LOGW(
          TAG,
          "GATTC sign_pk characteristic not found — aborting sign_pk exchange");
      (void)transport_ble_disconnect();
      break;
    }
    ...
    CeePewErr_t enc_err =
        gatt_crypto_encrypt_with_ids(session_code, g_ble_ctx.local_mac,
                                     g_ble_ctx.peer_mac, plaintext, encrypted);
    ceepew_secure_zero(plaintext, sizeof(plaintext));
    ceepew_secure_zero(session_code, sizeof(session_code));
    if (enc_err != CEEPEW_OK) {
      ESP_LOGE(TAG, "GATTC: sign_pk+box_pk encryption failed: %d",
               (int)enc_err);
    ...
    if (!g_ble_ctx.gattc_sign_pk_mtu_negotiated) {
      ESP_LOGI(
          TAG,
          "MTU not yet negotiated — buffering sign_pk write for CFG_MTU_EVT");
  ...
  case ESP_GATTC_WRITE_CHAR_EVT: {
    ...
    if (param->write.status == ESP_GATT_OK) {
      ...
      ESP_LOGI(TAG,
               "reverse_gattc_pending: 1->0 (GATTC write OK, init_sent=%d)",
               g_ble_ctx.initiator_sign_pk_sent);
      ...
      if (g_ble_ctx.sign_pk_write_attempts < 3U && g_ble_ctx.gattc_connected &&
         ...
         * Use 500 ms delay for first retry, 900 ms for second (exponential
         * backoff to prevent attp_build_sr_msg NULL p_msg race on rapid retry).
         */
        uint32_t retry_delay_ms =
            (g_ble_ctx.sign_pk_write_attempts == 1U) ? 500U : 900U;
        ESP_LOGW(TAG,
                 "GATTC sign_pk write status=133 (write attempt %u/3) — "
                 "retrying in %u ms",
        ...
        ESP_LOGW(
            TAG,
            "GATTC sign_pk write status=133 (retries=%u, write_attempts=%u) — "
      ...
      if (g_ble_ctx.sign_pk_write_attempts < 3U && g_ble_ctx.gattc_connected &&
        ...
        uint32_t retry_delay_ms =
            (g_ble_ctx.sign_pk_write_attempts == 1U) ? 500U : 900U;
        ESP_LOGW(TAG,
                 "GATTC sign_pk write failed: status=%d (write attempt %u/3) — "
                 "retrying in %u ms",
        ...
        ESP_LOGW(TAG,
                 "GATTC sign_pk write failed: status=%d — closing link, "
        ...
        ESP_LOGI(TAG,
                 "reverse_gattc_pending: 1->0 (GATTC write FAIL, status=%d)",
                 param->write.status);
        g_ble_ctx.reverse_gattc_pending = false;
        g_ble_ctx.sign_pk_write_attempts = 0U;
        (void)transport_ble_disconnect();
    ...
    if (!g_ble_ctx.commitment_verified && !g_ble_ctx.handoff_ready) {
      ...
      if (g_ble_ctx.handoff_ready) {
        ...
        if (g_ble_ctx.reverse_gattc_pending &&
            !g_ble_ctx.initiator_sign_pk_sent &&
            g_ble_ctx.reconnect_attempts < CEEPEW_MAX_RECONNECT_ATTEMPTS) {
          ESP_LOGI(TAG, "GATTC disconnected with rev_pend — scheduling reverse "
                        "GATTC reconnect");
          if (g_ble_ctx.peer_mac[0] != 0U) {
            CeePewErr_t err =
                transport_ble_retry_gattc_connect(g_ble_ctx.peer_mac);
            if (err != CEEPEW_OK) {
...
static void gatts_event_handler(esp_gatts_cb_event_t event,
  ...
  case ESP_GATTS_CREATE_EVT:
    ...
    /* Use explicit attribute value buffer for sign_pk characteristic
     * (GATT_CRYPTO_TOTAL_BYTES). The old NULL attr_value + NULL attr_control
     * approach used stack defaults which were too small for the 87-byte (1B ctr
     * + 70B ct + 16B tag) payload, causing ESP_GATT_INTERNAL_ERROR (133) on
     * GATT writes. */
    (void)esp_ble_gatts_add_char(
        param->create.service_handle, &s_sign_pk_char_uuid, ESP_GATT_PERM_WRITE,
        ESP_GATT_CHAR_PROP_BIT_WRITE, &s_sign_pk_attr_val_cfg, NULL);
    break;
  ...
  case ESP_GATTS_MTU_EVT:
    ...
    ESP_LOGI(
        TAG,
        "GATTS MTU negotiated: %u (need >= 90 for sign_pk+box_pk+wifi_mac)",
        (unsigned)g_ble_ctx.gatts_mtu);
    if (g_ble_ctx.gatts_mtu < GATT_MIN_MTU) {
      ESP_LOGW(TAG,
               "GATTS MTU %u below sign_pk+box_pk+wifi_mac threshold — peer "
               "write may fail",
               (unsigned)g_ble_ctx.gatts_mtu);
    }
    break;
  ...
  case ESP_GATTS_DISCONNECT_EVT:
    ...
    g_ble_ctx.connecting = false;
    ESP_LOGI(TAG,
             "GATTS disconnect: sign_pk_received=%d box_pk=%d "
    ...
    if (g_ble_ctx.handoff_ready) {
      ESP_LOGI(TAG,
               "GATTS disconnect: preserving sign_pk_received (handoff=%d)",
    ...
    bool should_handle_lost_gatts =
        (!g_ble_ctx.commitment_verified && !g_ble_ctx.handoff_ready);
  ...
  case ESP_GATTS_WRITE_EVT:
    if (param->write.need_rsp) {
      (void)esp_ble_gatts_send_response(gatts_if, param->write.conn_id,
    ...
    if (!param->write.is_prep &&
      ...
      /* Hybrid-GATT (Phase 7): the initiator's sign_pk+box_pubkey+wifi_mac is
       * now wrapped in Ascon-128 AEAD (70B ct + 16B tag = 86B). The key/nonce
       * are derived from the session_code, so a hostile GATTS device cannot
       * inject a chosen sign_pk without knowing the human-typed session code.
       *
       * THREAT MODEL:
      ...
      if (param->write.len != GATT_CRYPTO_TOTAL_BYTES) {
        ESP_LOGW(TAG,
                 "GATTS: sign_pk write length %u != %u (MTU %u?) — dropping",
                 (unsigned)param->write.len, (unsigned)GATT_CRYPTO_TOTAL_BYTES,
                 (unsigned)g_ble_ctx.gattc_mtu);
        break;
      }
      ...
      ESP_LOGI(
          TAG,
          "GATTS: sign_pk+box_pk write — local=%02X:%02X:%02X:%02X:%02X:%02X "
          "peer=%02X:%02X:%02X:%02X:%02X:%02X len=%u",
          g_ble_ctx.local_mac[0], g_ble_ctx.local_mac[1],
          g_ble_ctx.local_mac[2], g_ble_ctx.local_mac[3],
          g_ble_ctx.local_mac[4], g_ble_ctx.local_mac[5], g_ble_ctx.peer_mac[0],
          g_ble_ctx.peer_mac[1], g_ble_ctx.peer_mac[2], g_ble_ctx.peer_mac[3],
          g_ble_ctx.peer_mac[4], g_ble_ctx.peer_mac[5],
          (unsigned)param->write.len);
      CeePewErr_t dec_err = gatt_crypto_decrypt_with_ids(
          session_code, g_ble_ctx.local_mac, g_ble_ctx.peer_mac,
          param->write.value, decrypted);
      ...
      if (dec_err == CEEPEW_ERR_AUTH_FAIL || dec_err == CEEPEW_ERR_CRYPTO) {
        ESP_LOGW(TAG,
                 "GATTS: Ascon decrypt attempt 1 failed (%d) — retrying with "
                 "swapped MACs",
                 (int)dec_err);
        uint8_t sc_retry[32];
        ...
        if (sc_retry_err == CEEPEW_OK) {
          ...
          ESP_LOGI(TAG,
                   "GATTS retry DIAG: sc_hash=%02x%02x.. sc_byte0=%02x "
                   "local=%02x:%02x:%02x:%02x:%02x:%02x "
                   "peer=%02x:%02x:%02x:%02x:%02x:%02x",
                   sc_rhash[0], sc_rhash[1], sc_retry[0], g_ble_ctx.peer_mac[0],
                   g_ble_ctx.peer_mac[1], g_ble_ctx.peer_mac[2],
                   g_ble_ctx.peer_mac[3], g_ble_ctx.peer_mac[4],
                   g_ble_ctx.peer_mac[5], g_ble_ctx.local_mac[0],
                   g_ble_ctx.local_mac[1], g_ble_ctx.local_mac[2],
          ...
          CeePewErr_t dec2 = gatt_crypto_decrypt_with_ids(
              sc_retry, g_ble_ctx.peer_mac, g_ble_ctx.local_mac,
              param->write.value, decrypted);
          ceepew_secure_zero(sc_retry, sizeof(sc_retry));
          if (dec2 == CEEPEW_OK) {
            ESP_LOGI(TAG,
                     "GATTS: sign_pk decrypt SUCCEEDED on swapped-MAC retry");
            dec_err = CEEPEW_OK;
          } else {
            ESP_LOGW(TAG, "GATTS: swapped-MAC retry also failed (%d)",
                     (int)dec2);
          }
        } else {
          ESP_LOGW(TAG, "GATTS retry DIAG: session_code refetch failed: %d",
                   (int)sc_retry_err);
          ceepew_secure_zero(sc_retry, sizeof(sc_retry));
        }
      }
      ...
      if (dec_err == CEEPEW_ERR_AUTH_FAIL) {
        ESP_LOGW(TAG,
                 "GATTS: Ascon tag mismatch on sign_pk write — "
                 "dropping, bumping reconnect_attempts "
                 "(local=%02X%02X%02X peer=%02X%02X%02X)",
                 g_ble_ctx.local_mac[3], g_ble_ctx.local_mac[4],
                 g_ble_ctx.local_mac[5], g_ble_ctx.peer_mac[3],
                 g_ble_ctx.peer_mac[4], g_ble_ctx.peer_mac[5]);
      ...
      CeePewErr_t bp_err =
          session_set_peer_box_pubkey(decrypted + CEEPEW_ED25519_PUBKEY_BYTES);
      ...
      uint32_t peer_uptime_s =
          (uint32_t)decrypted[70U] | ((uint32_t)decrypted[71U] << 8U) |
          ((uint32_t)decrypted[72U] << 16U) | ((uint32_t)decrypted[73U] << 24U);
      ...
      ESP_LOGI(TAG, "GATTS: Peer uptime=%lu s, local=%lu s, offset=%ld s",
               (unsigned long)peer_uptime_s, (unsigned long)local_now_s,
               (long)peer_offset_s);
      ...
      if (pk_err == CEEPEW_OK && wifi_mac_err == CEEPEW_OK) {
        ...
        ESP_LOGI(TAG,
                 "GATTS: peer sign_pk SET (sign_pk_rcvd=%d box_pk=%d "
                 "is_init=%d rev_pend=%d init_sent=%d)",
                 g_ble_ctx.sign_pk_received, g_ble_ctx.box_pubkey_received,
                 g_ble_ctx.is_initiator_role, g_ble_ctx.reverse_gattc_pending,
                 g_ble_ctx.initiator_sign_pk_sent);
        transport_ble_post_event(PAIRING_EVENT_SIGN_PK_RECEIVED, NULL, 0U);
        ...
        if (!g_ble_ctx.is_initiator_role && !g_ble_ctx.initiator_sign_pk_sent &&
            !g_ble_ctx.reverse_gattc_pending) {
          g_ble_ctx.reverse_gattc_pending = true;
          ESP_LOGI(TAG, "reverse_gattc_pending: 0->1 (GATTS write received, "
                        "scheduling reverse)");
          ESP_LOGI(
              TAG,
              "GATTS: scheduling reverse GATTC for sign_pk+box_pk exchange");
        }
      } else {
...
/* Thread-safe helper: compute peer age (accumulated + current delta or
 * last_seen) Returns age in milliseconds. Safe to call from UI/task contexts.
 */
uint32_t transport_ble_get_peer_age_ms(void) {
  ...
  if (discovered && disc_start_ts != 0U) {
    age_ms = (now_ms > (disc_start_ts * 1000U))
                 ? (now_ms - (disc_start_ts * 1000U))
                 : 0U;
  }
...
static const char *transport_ble_phase_name(PairingPhase_t phase) {
  switch (phase) {
  case PAIRING_PHASE_IDLE:
    return "IDLE";
  case PAIRING_PHASE_DISCOVERY:
    return "DISCOVERY";
  case PAIRING_PHASE_BEACON_PAIRING:
    return "BEACON_PAIRING";
  case PAIRING_PHASE_GATT_IDENTITY:
    return "GATT_IDENTITY";
  case PAIRING_PHASE_SIGN_PK_EXCHANGE:
    return "SIGN_PK_EXCHANGE";
  case PAIRING_PHASE_FAILED:
    return "FAILED";
  default:
    return "UNKNOWN";
  }
...
static uint32_t transport_ble_phase_timeout_ms(PairingPhase_t phase) {
  switch (phase) {
  case PAIRING_PHASE_BEACON_PAIRING:
    return CEEPEW_PHASE_TIMEOUT_MS;
  case PAIRING_PHASE_GATT_IDENTITY:
    return CEEPEW_PHASE_TIMEOUT_GATT_MS;
  case PAIRING_PHASE_SIGN_PK_EXCHANGE:
    return CEEPEW_PHASE_TIMEOUT_CONNECT_MS;
  default:
    return CEEPEW_PHASE_TIMEOUT_MS;
  }
}

void transport_ble_post_event(PairingEventType_t type, const uint8_t *payload,
                              uint8_t payload_len) {
  ...
  ev.payload_len =
      (payload != NULL && payload_len <= sizeof(ev.payload)) ? payload_len : 0U;
...
void transport_ble_handle_connection_lost(void) {
  ...
  ESP_LOGI(
      TAG,
      "connection_lost: queued PHASE_TIMEOUT for supervisor re-evaluation");
}

static CeePewErr_t
transport_ble_handle_event_internal(const PairingEvent_t *event) {
  if (event == NULL) {
    return CEEPEW_ERR_NULL_PTR;
  }
  ...
  case PAIRING_EVENT_SIGN_PK_WRITE: {
    ...
    if (!g_ble_ctx.pending_sign_pk_write || !g_ble_ctx.gattc_connected) {
      ESP_LOGW(
          TAG,
          "SIGN_PK_WRITE: write no longer needed (pending=%u connected=%u)",
          (unsigned)g_ble_ctx.pending_sign_pk_write,
          (unsigned)g_ble_ctx.gattc_connected);
      ble_ctx_unlock();
      break;
    }
    if (g_ble_ctx.gattc_mtu < GATT_MIN_MTU) {
    ...
    ESP_LOGI(TAG, "SIGN_PK_WRITE: dispatching sign_pk write (delay=%u ms)",
             (unsigned)delay_ms);
    esp_err_t wr_err = esp_ble_gattc_write_char(
        g_ble_ctx.gattc_if, g_ble_ctx.conn_id,
        g_ble_ctx.gattc_sign_pk_char_handle,
        (uint16_t)sizeof(g_ble_ctx.pending_sign_pk_encrypted),
        g_ble_ctx.pending_sign_pk_encrypted, ESP_GATT_WRITE_TYPE_RSP,
        ESP_GATT_AUTH_REQ_NONE);
    if (wr_err != ESP_OK) {
      ESP_LOGW(TAG, "SIGN_PK_WRITE: write failed: %d (%s)", (int)wr_err,
  ...
  case PAIRING_EVENT_SIGN_PK_RECEIVED: {
    ESP_LOGI(TAG,
             "supervisor: SIGN_PK_RECEIVED — phase=%s handoff=%d sign_pk=%d",
             transport_ble_phase_name(s_pairing_ctx.phase),
  ...
  while (s_supervisor_running) {
    ...
    if ((now_ms - last_metrics_log) >= 5000U) {
      ...
      if (phase == PAIRING_PHASE_GATT_IDENTITY ||
        ...
        ble_ctx_lock();
        ESP_LOGI(TAG,
                 "GATT Metrics: phase=%s open=%u mtu=%u write=%u disc=%u "
                 "reason=0x%02X "
...
static void transport_ble_supervisor_check_stall(void) {
  ...
  if (elapsed > budget) {
    ESP_LOGW(TAG,
             "supervisor: phase %s stalled for %u ms (budget %u, entered=%u, "
             "now=%u)",
             transport_ble_phase_name(phase), (unsigned)elapsed,
             (unsigned)budget, (unsigned)entered, (unsigned)now_ms);
    transport_ble_post_event(PAIRING_EVENT_PHASE_TIMEOUT, NULL, 0U);
  }
}
...
static void transport_ble_supervisor_tick_adv_watchdog(void) {
  ...
  if (adv) {
    ...
    if (s_adv_down_since_ms == 0U) {
      ...
      ESP_LOGW(TAG,
               "adv watchdog: advertising down for %u ms — forcing re-arm "
               "(phase=%s state=%s)",
...
static void transport_ble_supervisor_tick_recovery(void) {
  if (s_recovery_state == RECOVERY_STATE_IDLE) {
    return;
  }
  ...
  uint32_t now_ms = (uint32_t)(esp_timer_get_time() / 1000ULL);
  uint32_t elapsed =
      (now_ms >= s_recovery_started_ms) ? (now_ms - s_recovery_started_ms) : 0U;
  ...
  case RECOVERY_STATE_VERIFY_RESTART:
    ...
    }
    {
      ...
      if (post_state == BLE_ADVERTISING || post_state == BLE_SCANNING ||
...
CeePewErr_t transport_ble_supervisor_start(void) {
  ...
  if (g_pairing_event_queue == NULL) {
    g_pairing_event_queue =
        xQueueCreate(CEEPEW_PAIRING_EVENT_QUEUE_DEPTH, sizeof(PairingEvent_t));
    if (g_pairing_event_queue == NULL) {
      return CEEPEW_ERR_HW;
    }
  }
  ...
  BaseType_t ok = xTaskCreate(
      transport_ble_supervisor_task, "pair_supp", CEEPEW_SUPERVISOR_STACK_BYTES,
      NULL, CEEPEW_SUPERVISOR_PRIORITY, &s_supervisor_task_handle);
  if (ok != pdPASS) {
...
RgbPattern_t transport_ble_phase_to_rgb(PairingPhase_t phase) {
  ...
  case PAIRING_PHASE_IDLE:
    return RGB_OFF;
  case PAIRING_PHASE_DISCOVERY:
    return RGB_BLUE_PULSE;
  case PAIRING_PHASE_BEACON_PAIRING:
    return RGB_AMBER_PULSE;
  case PAIRING_PHASE_GATT_IDENTITY:
    return RGB_CYAN_BLINK;
  case PAIRING_PHASE_SIGN_PK_EXCHANGE:
    return RGB_CYAN_PULSE;
  case PAIRING_PHASE_FAILED:
    return RGB_RED_BLINK;
  default:
    return RGB_OFF;
  }
}
...
bool transport_ble_is_recovering(void) { return s_supervisor_recovering != 0U; }
