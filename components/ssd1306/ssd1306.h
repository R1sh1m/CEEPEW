...
/* Forward-declare the SPI device handle so this header compiles in I2C-only
 * builds without pulling in the full spi_master.h (and the SPI driver).
 * CEE-PEW never instantiates the SPI path. */
struct spi_device_t;
typedef struct spi_device_t *spi_device_handle_t;

#include "esp_idf_version.h"
/* CEE-PEW targets ESP-IDF v6.0.1, so the new I2C master driver is always
 * available. The legacy driver/i2c.h was removed in v6. */
