...
/* CEE-PEW: project-wide log tag used by the new helper functions at the
 * bottom of this file. The upstream functions use __FUNCTION__ as the tag. */
static const char *TAG_SSD = "SSD1306";
...
/* ====================================================================
 * CEE-PEW extensions (not in upstream)
 * ==================================================================== */

/*
 * ssd1306_set_charge_pump() — turn the SSD1306 internal DC-DC charge pump
 * on or off. Most 0.96" modules with no external VCC need this ON to
 * produce enough drive voltage at 3.3V. CEE-PEW toggles it twice during
 * the boot self-test (lines 100-114 of main/main.c) as a visual
 * confirmation that the I2C link is alive.
 */
esp_err_t ssd1306_set_charge_pump(SSD1306_t * dev, bool enabled)
{
	if (dev == NULL) { return ESP_ERR_INVALID_ARG; }
	uint8_t out_buf[4];
	out_buf[0] = OLED_CONTROL_BYTE_CMD_STREAM;   /* 0x00 */
	out_buf[1] = OLED_CMD_SET_CHARGE_PUMP;       /* 0x8D */
	out_buf[2] = enabled ? 0x14U : 0x10U;        /* 0x14 = enable, 0x10 = disable */
	out_buf[3] = OLED_CMD_DISPLAY_ON;            /* 0xAF — keep display on */
	esp_err_t res = i2c_master_transmit(dev->_i2c_dev_handle, out_buf, sizeof(out_buf), 100);
	if (res != ESP_OK) {
		ESP_LOGE(TAG_SSD, "ssd1306_set_charge_pump(%d) failed: %d (%s)",
		         (int)enabled, (int)res, esp_err_to_name(res));
	}
	return res;
}

/*
 * ssd1306_selftest_flash() — issue the SSD1306 "Entire Display ON" command
 * (0xA5) for `duration_ms`, then "Resume to RAM" (0xA4). Visually flashes
 * the panel. Clamps to [1, 3000] ms for safety.
 */
esp_err_t ssd1306_selftest_flash(SSD1306_t * dev, uint32_t duration_ms)
{
	if (dev == NULL) { return ESP_ERR_INVALID_ARG; }
	if (duration_ms == 0U) { return ESP_OK; }
	if (duration_ms > 3000U) { duration_ms = 3000U; }

	const uint8_t on_buf[2]  = { OLED_CONTROL_BYTE_CMD_STREAM, OLED_CMD_DISPLAY_ALLON }; /* 0xA5 */
	const uint8_t off_buf[2] = { OLED_CONTROL_BYTE_CMD_STREAM, OLED_CMD_DISPLAY_RAM };   /* 0xA4 */

	esp_err_t res = i2c_master_transmit(dev->_i2c_dev_handle, on_buf, sizeof(on_buf), 100);
	if (res != ESP_OK) {
		ESP_LOGE(TAG_SSD, "ssd1306_selftest_flash (on) failed: %d (%s)", (int)res, esp_err_to_name(res));
		return res;
	}
	vTaskDelay(pdMS_TO_TICKS(duration_ms));
	res = i2c_master_transmit(dev->_i2c_dev_handle, off_buf, sizeof(off_buf), 100);
	if (res != ESP_OK) {
		ESP_LOGE(TAG_SSD, "ssd1306_selftest_flash (off) failed: %d (%s)", (int)res, esp_err_to_name(res));
	}
	return res;
}
