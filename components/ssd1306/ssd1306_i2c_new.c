...
/* CEE-PEW override: project never sets CONFIG_OFFSETX via Kconfig, and the
 * SH1106 column-shift workaround lives in hal_ui.c. Force a literal 0 here
 * to keep the vendored code compiling without dragging in Kconfig plumbing. */
...
void i2c_master_init(SSD1306_t * dev, int16_t sda, int16_t scl, int16_t reset)
{
	...
	i2c_master_bus_config_t i2c_mst_config = {
		...
		.sda_io_num = sda,
		/*
		 * CEE-PEW modification:
		 *   trans_queue_depth = 0 selects the I2C master's synchronous
		 *   mode. Every i2c_master_transmit() blocks until the physical
		 *   transfer finishes before returning; the async queue can never
		 *   overflow because there is no queue. The driver still yields
		 *   to other tasks (BLE, UI) via its internal semaphore during the
		 *   byte-by-byte transfer, so no task starvation.
		 *   This matches the behavior of the previous CEE-PEW OLED driver.
		 */
		.trans_queue_depth = 0,
...
void i2c_display_image(SSD1306_t * dev, int page, int seg, const uint8_t * images, int width) {
	...
	uint8_t *out_buf;
	/*
	 * CEE-PEW note: this upstream function uses malloc/free. CEE-PEW's
	 * project rule forbids heap allocation outside the region allocator,
	 * so the high-level flush path (hal_ui_flush) implements its own
	 * no-malloc equivalent. We do not call i2c_display_image() from
	 * project code; it is here only for the upstream ssd1306_show_buffer
	 * and ssd1306_bitmaps paths, which CEE-PEW does not use.
	 */
